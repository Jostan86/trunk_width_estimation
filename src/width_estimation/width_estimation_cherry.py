...
class TrunkSegmenter:
    def __init__(self, package_paths):
        ...
        with open(package_paths.config_file_path) as f:
...
class TrunkAnalyzer:
    def __init__(self, config_file_path, combine_segmenter=False):
    ...
    def get_width_pix(self, mask, idx, top_ignore=0.6, bottom_ignore=0.1, segment_length=25):
        ...
        self.img_x_positions[idx] = int(np.mean(np.where(mask)[1]))
        ...
        # Keep only rows where a mask is present, which is rows where the leftmost pixel is not zero and right most is not
        ...
        # Draw the leftmost and rightmost points on the mask
        mask_with_points = np.copy(mask)
        for i in range(len(y_coords)):
            mask_with_points[y_coords[i], leftmost[i]] = 255
            mask_with_points[y_coords[i], rightmost[i]] = 255

        # Draw the midpoints on the mask
        for i in range(len(y_coords)):
            mask_with_points[y_coords[i], x_coords[i]] = 255
        ...
        # print corrected_widths in order of value
        corrected_widths = np.sort(corrected_widths)
        # remove any zeros from the array
        corrected_widths = corrected_widths[corrected_widths != 0]

        return corrected_widths


    def calculate_depth(self, top_ignore=0.5, bottom_ignore=0.10, min_num_points=300,
    ...
    def calculate_width(self):
        ...
        for i, (mask, depth) in enumerate(zip(self.masks, self.depth_median)):
            ...
            diameter_pixels = self.get_width_pix(mask, i)
            ...
            self.tree_widths[i] = np.percentile(diameter_m, 60)
            
            # Add half the width to the depth and repeat the calculations
            depth += self.tree_widths[i] / 2
            image_width_m = depth * np.tan(np.deg2rad(horz_fov / 2)) * 2
            distperpix = image_width_m / self.original_width
            diameter_m = diameter_pixels * distperpix
            ...
            self.tree_locations[i, 1] = depth
            self.tree_locations[i, 0] = (self.img_x_positions[i] - (self.original_width / 2)) * distperpix
    ...
    def process_image(self):
        ...
        if self.num_instances > 0:
            ...
            #                      depth_filter_percentile_upper=65, depth_filter_percentile_lower=35)
            self.calculate_depth(top_ignore=0.60, bottom_ignore=0.20, min_num_points=300,
                            depth_filter_percentile_upper=70, depth_filter_percentile_lower=30)
        ...
        if self.num_instances > 0:
            self.mask_filter_position(bottom_position_threshold=0.2, top_position_threshold=0.65, score_threshold=0.9)
    ...
    def map_making_helper(self, rgb_img, depth_img, show_seg=False):
        ...
        self.tree_widths = self.tree_widths - 2.8e-05 * img_x_positions_rel_middle
    ...
    def get_mask_and_seg(self, rgb_img, depth_img):
        ...
        return self.masks, results.plot(), self.classes
        

if __name__ == "__main__":
    # actual: 19.2
    # rgb_img_path = "/media/jostan/portabits/map_making_data/cherry_orchard/images/row_3/rgb/1708117173_904114007.png"
    # depth_path = "/media/jostan/portabits/map_making_data/cherry_orchard/images/row_3/depth/1708117173_904114007.png"
    # actual: 15.5
    rgb_img_path = "/media/jostan/portabits/map_making_data/cherry_orchard/images/row_3/rgb/1708117180_708707094.png"
    depth_path = "/media/jostan/portabits/map_making_data/cherry_orchard/images/row_3/depth/1708117180_708707094.png"
    rgb_img = cv2.imread(rgb_img_path)
    depth_img = cv2.imread(depth_path, cv2.IMREAD_ANYDEPTH)
    trunk_analyzer = TrunkAnalyzer(combine_segmenter=True)

    _ = trunk_analyzer.map_making_helper(rgb_img, depth_img, show_seg=True)
