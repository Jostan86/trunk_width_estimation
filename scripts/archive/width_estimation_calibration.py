...
class TrunkAnalyzer:
    ...
    #     cut_off_dist = int(corrected_widths.shape[0] * 0.1)
    ...
    def get_width_pix(self, mask, idx, top_ignore=0.1, bottom_ignore=0.1, segment_length=25):
        """Get the width of the segmentation in pixels. Note that this method is different than in the paper, but
        it produces a similar results and is significantly faster."""
    ...
    #
    #         print("diameter_pixels: ", np.mean(diameter_pixels))
    #
    ...
    def calculate_width(self):
        ...
        for i, (mask, depth) in enumerate(zip(self.masks, self.depth_median)):
            ...
            print("Pixel width: ", np.mean(diameter_pixels))
            ...
            self.tree_widths[i] = np.percentile(diameter_m, 50)
    ...
    def process_image(self):
        ...
        if self.num_instances > 0:
            ...
            #                      depth_filter_percentile_upper=65, depth_filter_percentile_lower=35)
            self.calculate_depth(top_ignore=0.0, bottom_ignore=0.0, min_num_points=200,
